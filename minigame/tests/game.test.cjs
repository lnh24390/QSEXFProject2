...
function runTests() {
  ...
  test('final boss defeat wins once, stops hazards and progression, and restart resets victory', () => {
    for (const mode of Object.keys(MODES)) {
      const g = fresh(mode); g.round = 100; g.spawnBoss(); g.fireTimer = 100;
      g.boss.hp = 0; g.invincible = 0; g.lives = 1;
      g.hostile.push({ x: g.player.x, y: g.player.y, vx: 0, vy: 0, r: 7, homing: true });
      g.takeEvents(); g.update(.01);
      assert.equal(g.won, true); assert.equal(g.state, 'over'); assert.equal(g.lives, 1);
      assert.equal(g.round, 100); assert.equal(g.boss, null); assert.equal(g.nextBoss, Infinity);
      assert.equal(g.hostile.length, 0); assert.equal(g.score, 2500);
      assert.equal(g.takeEvents().filter(e => e.type === 'over').length, 1);
      const elapsed = g.elapsed; g.update(.05); g.resume();
      assert.equal(g.elapsed, elapsed); assert.equal(g.state, 'over'); assert.equal(g.takeEvents().length, 0);
      g.reset(mode); assert.equal(g.won, false); assert.equal(g.round, 1); assert.equal(g.state, 'playing');
    }
  });
