...
function runTests() {
  ...
  test('final victory shows release notice and restarts from round one on desktop and mobile', () => {
    for (const width of [320, 1280]) {
      const p = createPage({ width }); p.key('Enter');
      p.game.round = 100; p.game.spawnBoss(); p.game.boss.hp = 0; p.frames(2);
      assert.equal(p.game.won, true); assert.equal(p.game.state, 'over');
      assert.ok(p.get('panel').innerHTML.includes('당신의 승리입니다'));
      assert.ok(p.get('panel').innerHTML.includes('추후 정식 출시 시 Android 버전으로 찾아뵙도록 하겠습니다.'));
      assert.ok(p.get('panel').innerHTML.includes('다시하기'));
      assert.equal(p.get('overlay').classList.contains('hidden'), false);
      assert.equal(p.get('pause').style.display, 'none');
      const elapsed = p.game.elapsed; p.frames(120); assert.equal(p.game.elapsed, elapsed);
      p.get('restart').emit('click');
      assert.equal(p.game.round, 1); assert.equal(p.game.won, false); assert.equal(p.game.state, 'playing');
    }
  });
  ...
  test('menu describes every difficulty and the upper-left pause button pauses and resumes', () => {
