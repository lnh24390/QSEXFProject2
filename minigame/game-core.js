...
(function (root, factory) {
  ...
  class Game {
    ...
    reset(mode = this.mode) {
      ...
      Object.assign(this, { state: 'playing', won: false, elapsed: 0, score: 0, lives: this.config.lives, maxLives: this.config.lives,
    ...
    update(dt) {
      ...
      for (const b of [...this.activeBosses]) {
        ...
        if (b.hp <= 0) {
          ...
          if (this.round === 100 && !this.boss) {
            this.won = true; this.state = 'over'; this.clearInput();
            this.hostile = []; this.bullets = []; this.nextBoss = Infinity;
            this.emit('over'); return;
          }
