(() => {
  ...
  function chooseMode(next) {
    ...
    const descriptions = {
      chill: '체력 5 · 느린 적과 여유로운 등장 간격 · 일반 보스 1대',
      arcade: '체력 3 · 기본 속도의 균형 잡힌 전투 · 일반 보스 1대',
      expert: '체력 2 · 빠른 적과 짧은 등장 간격 · 일반 보스 1대',
      impossible: '체력 1 · 하드코어 속도 · 일반 보스 2대 동시 등장'
    };
    $('mode-description').textContent = `${MODES[mode].label} — ${descriptions[mode]}`;
    $('mode-items').textContent = mode === 'impossible'
      ? '쉴드: 보스 한 대 격파마다 1개 · 10라운드마다 웨이브 보스 1대'
      : '쉴드: 15초마다 1개 · 10라운드마다 웨이브 보스 1대';
    $('mode-items').textContent += ' · 100스테이지 최종보스: 3단계 공격 + 5초마다 유도미사일 1발. 미사일은 플레이어와 접촉할 때까지 추적합니다.';
  }
  function bindMenu() {
  ...
  function start() {
    ...
    announce('빛을 모으세요 · 30초 뒤 첫 보스'); updateHUD(); tone(660, .2);
  }
  function home() {
  ...
  function gameOver() {
    ...
    $('overlay').classList.remove('hidden');
    $('panel').innerHTML = `<div class="badge"><i></i> ${MODES[mode].label} / RUN COMPLETE</div><h2 class="results-title">${game.won ? '당신의 승리입니다' : '멋진 드리프트였어요.'}</h2>${game.won ? '<p class="release-notice">추후 정식 출시 시 Android 버전으로 찾아뵙도록 하겠습니다.</p>' : ''}<div class="results-score">${pad(game.score)}</div><div class="new-record">${record ? '✦ 새로운 최고 기록!' : '한 번 더, 조금 더 멀리.'}</div><div class="result-grid"><div><strong>${timeLabel(game.elapsed)}</strong><small>생존 시간</small></div><div><strong>${game.kills}</strong><small>적 격파</small></div><div><strong>${game.maxCombo}</strong><small>최대 콤보</small></div></div><p class="result-summary">보스 ${game.bosses}회 격파 · 크리스털 ${game.crystals}개</p><button class="start-button" id="restart">${game.won ? '다시하기' : '다시 도전'} <span>↗</span></button><button class="secondary-button" id="home">난이도 선택</button>`;
    $('restart').addEventListener('click', start); $('home').addEventListener('click', home); $('restart').focus({ preventScroll: true });
    updateHUD(); tone(game.won ? 880 : 100, .5, game.won ? 'triangle' : 'sawtooth', .04);
  }
  const moveKeys = ['arrowup', 'arrowdown', 'arrowleft', 'arrowright', 'w', 'a', 's', 'd'];
  const physicalMoveKeys = { KeyW: 'w', KeyA: 'a', KeyS: 's', KeyD: 'd' };
  const keyboardKey = event => physicalMoveKeys[event.code] || event.key.toLowerCase();
  ...
  window.addEventListener('keyup', event => { const k = keyboardKey(event); if (keys.delete(k)) keyboardMove(); });
  ...
  function updateHUD() {
    ...
    updateHealth(game.lives, game.maxLives);
    ...
    $('power-status').textContent = [game.shield ? `◈ 보호막 ${Math.ceil(game.shield)}s (1회)` : '', game.spreadTime > 0 ? `확산탄 ${Math.ceil(game.spreadTime)}s` : ''].filter(Boolean).join(' · ') || '기본 사격';
    $('sector').textContent = `ROUND ${String(game.round).padStart(2, '0')} / ${game.boss ? 'BOSS FIGHT' : `BOSS IN ${Math.max(0, Math.ceil(game.nextBoss - game.elapsed))}s`}`;
    ...
    const waveBoss = game.boss?.kind === 'wave' ? game.boss : null;
    $('wave-health').hidden = !waveBoss;
    $('boss-track').hidden = !!waveBoss;
    for (let index = 0; index < 3; index++) {
      const percent = waveBoss ? clamp(waveBoss.waveHealth[index] / waveBoss.waveMaxHP * 100, 0, 100) : 0;
      $('wave-fill-' + (index + 1)).style.width = `${percent}%`;
      $('wave-percent-' + (index + 1)).textContent = `${Math.ceil(percent)}%`;
      $('wave-track-' + (index + 1)).setAttribute('aria-valuenow', String(Math.ceil(percent)));
      $('wave-row-' + (index + 1)).classList.toggle('active', !!waveBoss && waveBoss.wave === index + 1);
      $('wave-row-' + (index + 1)).classList.toggle('cleared', percent === 0);
    }
    const dualBoss = game.boss?.laneCount === 2;
    $('boss-second').hidden = !dualBoss;
    for (let lane = 0; lane < (dualBoss ? 2 : 1); lane++) {
      const suffix = lane ? '-2' : '';
      const boss = game.activeBosses.find(b => b.lane === lane);
      const percent = boss ? clamp(boss.hp / boss.maxHP * 100, 0, 100) : 0;
      const name = dualBoss ? (lane ? '오른쪽 보스' : '왼쪽 보스') : boss?.kind === 'wave' ? `WAVE ${boss.wave} / 3` : 'THE SENTINEL';
      $('boss-name' + suffix).textContent = name;
      $('boss-fill' + suffix).style.width = `${percent}%`;
      $('boss-percent' + suffix).textContent = boss ? `${Math.ceil(percent)}%` : '격파';
      $('boss-track' + suffix).setAttribute('aria-label', `${name} 체력`);
      $('boss-track' + suffix).setAttribute('aria-valuenow', String(Math.ceil(percent)));
    }
  }
  function handleEvents() {
    for (const event of game.takeEvents()) {
      ...
      if (type === 'pickup') {
        ...
        announce({ spread: '확산탄 가동 · 10초', shield: '보호막 장착 · 15초 / 피격 1회 방어', 'max-health': '최대 체력 +1 · 체력 +1 회복!', heal: healMessage }[event.kind]);
        if (event.kind === 'heal') {
          labels.push({ x: game.player.x, y: game.player.y - 26, text: event.healed ? 'HP +1' : '+250', life: 1.2, color: '#b5ffda' });
          updateHUD();
        }
        if (event.kind === 'max-health') updateHUD();
        tone(1300, .25);
      }
      if (type === 'max-health-drop') announce('웨이브 보스 격파 · 최대 체력 +1 · 5초 안에 획득하세요!');
      if (type === 'heal-drop') announce('회복 캡슐 등장 · 초록색 +를 모으세요');
      if (type === 'sector') announce(`SECTOR ${String(event.sector).padStart(2, '0')} — KEEP GOING`);
      if (type === 'boss') { announce(event.kind === 'wave' ? 'WARNING / 3-WAVE BOSS' : event.count === 2 ? 'WARNING · 센티널 2대 접근 중' : 'WARNING · 센티널 접근 중'); tone(150, .7, 'triangle'); }
      if (type === 'boss-wave') announce(`WAVE ${event.wave} / 3`);
      if (type === 'boss-down') { burst(x, y, '#ffba85', 80, 310); burst(x, y, '#b5ffda', 50, 230); shake = reduced ? 0 : 7; announce(game.won ? '당신의 승리입니다' : event.remaining ? 'BOSS DOWN · 남은 보스 1대' : 'BOSS CLEAR · 다음 전투까지 30초'); tone(880, .5, 'triangle'); }
  ...
  function render(dt) {
    ...
    if (game.state !== 'menu') {
      ...
      for (const item of game.pickups) {
        const color = { spread: '#d1a6ff', shield: '#81dfff', heal: '#b5ffda', 'max-health': '#ffd46b' }[item.kind];
        if (item.kind === 'max-health') {
          polygon(item.x, item.y, 23, 6, 0, color, '#3b2d16');
          ctx.save(); ctx.fillStyle = color; ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'center';
          ctx.fillText('+1', item.x, item.y + 4); ctx.font = 'bold 9px sans-serif';
          ctx.fillText(`MAX HP +1 / ${Math.max(0, Math.ceil(item.expiresAt - game.elapsed))}s`, item.x, item.y + 37); ctx.restore();
      ...
      for (const b of game.activeBosses) { if (b.kind === 'wave') {
        polygon(b.x, b.y, b.r, 8, b.phase * .2, '#d1a6ff', '#36204e');
        for (const offset of [-22, 0, 22]) polygon(b.x + offset, b.y + 15, 11, 3, Math.PI, '#81dfff', '#352756');
        polygon(b.x, b.y - 8, 13, 4, b.phase, '#fff0d2', '#9b65ff80');
        continue;
      } polygon(b.x, b.y, b.r, 6, b.phase * .2, '#ff8c7f', '#421e3570'); polygon(b.x, b.y, b.r * .65, 6, -b.phase * .5, '#e9a7ff', '#b672ff20'); polygon(b.x, b.y, 12, 4, b.phase, '#fff0d2', '#ff866080'); polygon(b.x - 51, b.y + 10, 14, 3, -.4, '#ff8c7f', '#421e35'); polygon(b.x + 51, b.y + 10, 14, 3, .4, '#ff8c7f', '#421e35'); }
