(() => {
  ...
  const storageKey = key => window.NEON_WAVE_TEST ? `wave-test-${key}` : key;
  const saved = (key, fallback) => { try { return localStorage.getItem(storageKey(key)) ?? fallback; } catch { return fallback; } };
  const save = (key, value) => { try { localStorage.setItem(storageKey(key), value); } catch {} };
  ...
  function start() {
    ...
    announce(window.NEON_WAVE_TEST ? 'WAVE BOSS TEST' : '빛을 모으세요 · 30초 뒤 첫 보스'); updateHUD(); tone(660, .2);
  }
  function home() {
  ...
  function handleEvents() {
    for (const event of game.takeEvents()) {
      ...
      if (type === 'boss-down') { burst(x, y, '#ffba85', 80, 310); burst(x, y, '#b5ffda', 50, 230); shake = reduced ? 0 : 7; announce(event.remaining ? 'BOSS DOWN · 남은 보스 1대' : 'BOSS CLEAR · 다음 전투까지 30초'); tone(880, .5, 'triangle'); }
